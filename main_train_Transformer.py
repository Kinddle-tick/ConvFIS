...
from model import iTransformer, Transformer
...
ModelConfig = namedtuple('Config', [
    ...
    'e_layers', 'd_ff', 'activation', 'channel_independence', 'c_out', 'dec_in', 'enc_in', 'd_layers'
])
...
if __name__ == '__main__':
    ...
    # 读取配置文件
    load_config()
    cfg = ConfigManager("default.ini", ).parser("model_Transformer")
    ...
    model.seq_len = model_config.seq_len
    ...
    lr_scheduler = get_cosine_with_hard_restarts_schedule_with_warmup(  
        optimizer=optimizer,  
        num_warmup_steps=30,            # 预热步数
        num_cycles= 10,  
        num_training_steps=num_training_steps  
) 

    # # 定义训练参数
    # args = (TrainingArguments(sys.argv, valid_interval=10, save_dir_prefix=save_dir_prefix)
    ...
    trainer = Trainer(model, cfg.train_args, train_dataset=cfg.train_dataset, eval_dataset=cfg.valid_dataset, optimizers=(optimizer, lr_scheduler))
